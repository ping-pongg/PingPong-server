...
public class LlmQaService {
   ...
   """;
	 ...
	 * 최종 시나리오 생성 실행 메서드
	 */
	public QaOutcome generateScenarios(EndpointAggregate spec) {
		...
		try {
			...
			Map<String, Object> simplifiedSpec = Map.of(
				...
				"parameters", spec.parameters().stream()
					.map(p -> Map.of("name", p.getName(), "in", p.getInType(), "required", p.getRequired()))
					.toList(),
			...
			// 2. 고정된 유저 프롬프트 (Body 생성을 강제하는 지시사항 포함)
            ...
            [분석할 API 명세]
            %s
            """, spec.endpoint().getId(), spec.endpoint().getPath(), swaggerJson);
